import { GenerateCanvas, THREE, OrbitControls, GLTFLoader, CreateParticles } from './study/settings';
...
for (let i = 0; i < 1000; i++) {
  positions.push((Math.random() * 2 - 1) * 30); // x
...
for (let i = 0; i < 300; i++) {
  cloudPositions.push((Math.random() * 2 - 1) * 50); // x
...
cloudParticlePosition.setAttribute('position', new THREE.BufferAttribute(new Float32Array(cloudPositions), 3));
...
function render() {
  ...
  for (let i = 0; i < 1000; i++) {
    const yIndex = i * 3 + 1;
    particlePosition.attributes.position.array[yIndex] = positions[yIndex] + 10 * Math.sin(i + time);
  }
