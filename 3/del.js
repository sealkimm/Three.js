import { GenerateCanvas, THREE, GLTFLoader, OrbitControls } from './study/settings';
  ...
  loader.load('/gltf/clock.glb', function (gltf) {
    ...
    model.traverse(function (child) {
      ...
      if (child.name.toLowerCase().includes('camera')) {
        ...
        camera.position.set(child.position.x * ratio, child.position.y * ratio, child.position.z * ratio);
...
function setTime() {
  ...
  if (date.getHours() > 6 && date.getHours() < 18) {
    // day
    currentSky = color.day.sky;
    currentGround = color.day.ground;
    currentBackground = color.day.background;
  } else {
    // night
    currentSky = color.night.sky;
    currentGround = color.night.ground;
    currentBackground = color.night.background;
  }

  scene.background = new THREE.Color(currentBackground);
  hemiLight.color = new THREE.Color(currentSky);
  hemiLight.ground = new THREE.Color(currentGround);
}
